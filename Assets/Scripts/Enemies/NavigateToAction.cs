...
[NodeDescription(name: "Navigate To", story: "[Agent] navigates to [PlayerTransform]", category: "Action", id: "ea9381f629df24981a1c424f902e318d")]
public partial class NavigateToAction : Action
{
    ...
    protected override Status OnUpdate()
    {
        ...
        bool boolUpdateTargetPosition = !Mathf.Approximately(m_LastTargetPosition.x, PlayerTransform.Value.transform.position.x)
            || !Mathf.Approximately(m_LastTargetPosition.y, PlayerTransform.Value.transform.position.y)
            || !Mathf.Approximately(m_LastTargetPosition.z, PlayerTransform.Value.transform.position.z);

        if (boolUpdateTargetPosition)
        {
            m_LastTargetPosition = PlayerTransform.Value.transform.position;
    ...
    protected float SimpleMoveTowardsLocation(Transform agent, Vector3 target, float speed, float distance, float slowDownDistance)
    {
        // Direction vector from agent to target
        Vector3 direction = target - agent.position;

        // Distance to target (use passed 'distance' if you want external calculation)
        ...
        // Adjust speed if within slow down range
        float finalSpeed = speed;
        if (slowDownDistance > 0f && remainingDistance < slowDownDistance)
        {
            finalSpeed *= remainingDistance / slowDownDistance;
        }

        // Move agent
        agent.position += direction * finalSpeed * Time.deltaTime;
    ...
    private Vector3 GetPositionColliderAdjusted()
    {
        switch (m_TargetPositionMode.Value)
        {
            case TargetPositionMode.ClosestPointOnAnyCollider:
                Collider anyCollider = PlayerTransform.Value.GetComponentInChildren<Collider>(includeInactive: false);
            ...
            case TargetPositionMode.ClosestPointOnTargetCollider:
                Collider targetCollider = PlayerTransform.Value.GetComponent<Collider>();
        ...
        return PlayerTransform.Value.transform.position;
    }
